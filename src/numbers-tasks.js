...
function getAverage(value1, value2) {
  let total;
  if (value1 === Number.MIN_VALUE && value2 === Number.MIN_VALUE) {
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const total = Math.abs(
    Math.abs(Math.atan2(x1, y1)) - Math.abs(Math.atan2(x2, y2))
  );
  return total;
}
...
function roundToPowerOfTen(num, pow) {
  ...
  if (pow > 0) {
    ...
    total = Number(String(Math.round(num / Number(factor))) + factor.slice(1));
  } else if (pow === 0) {
    total = num;
  }
...
function getSumOfDigits(num) {
  ...
  const rez = arr.reduce((sum, current) => sum + Number(current), 0);
  return rez;
}
...
function getCountOfOddNumbers(number) {
  const rez = Math.round(Math.abs(number) / 2);
  return rez;
}
